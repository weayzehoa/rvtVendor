...
<body class="hold-transition login-page bg-navy" style="background-image: url({{ asset('img/icarry-index-cover-20191210.jpg') }});">
    ...
    <div class="login-box">
        <div class="text-lg text-center mb-2">
            <a href="javascript:" class="text-white"><b>@if(env('APP_ENV') == 'local')開發團隊測試用<br>@endif 帳號密碼重新設定</b></a>
        </div>
        <div class="card">
            <div class="card-body login-card-body">
                <p class="login-box-msg">請輸入 帳號 與 Email</p>
                <form id="forgetForm" action="{{ route('vendor.forget.submit') }}" method="post">
                    @csrf
                    ...
                    <div class="input-group mb-3">
                        <input id="email" type="email" placeholder="請輸入Email" class="form-control {{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" required>
                        <div class="input-group-append">
                            <div class="input-group-text">
