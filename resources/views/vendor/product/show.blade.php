    ...
    <section class="content">
        <div class="container-fluid">
            <div class="card card-primary card-outline">
                <div class="card-body">
                    <div class="mb-2">
                        <span class="text-danger text-bold">@if($viewOnly == 1)目前為查看模式，無法編輯修改，請重新點擊 <a href="{{ route('vendor.product.show',$product->id) }}">這裡</a> 轉為編輯模式。 @endif @if(isset($product) && !isset($copy))商品狀態為 上架中、補貨中、待審中無法修改資料。@else 注意!! 商品名稱、單價、毛重、淨重及內容(規格)儲存後將無法修改。若需修改，請與 iCarry 聯繫。@endif</span><br>
                    </div>
                    @if(isset($product) && !isset($copy))
                    ...
                    @endif
                        ...
                        <div class="row">
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-6">
                                        @if(!in_array($product->status,[1,-3]))
                                            ...
                                            <h3 class="text-danger">{{ !empty($product->verification_reason) ? $product->verification_reason.':'.$product->pause_reason : '無' }}</h3>
                                        @else
                                        @if(!empty($product->pause_reason))
                                    ...
                                    <div class="form-group col-12">
                                        ...
                                        <input type="text" class="form-control {{ $errors->has('brand') ? ' is-invalid' : '' }}" id="brand" {{ !isset($copy) && isset($product) && in_array($product->status,[1,-3]) ? 'disabled' : 'name=brand' }} value="{{ old('brand') ?? $product->brand ?? '' }}" placeholder="輸入廠牌名稱">
                                        @if ($errors->has('brand'))
                                    ...
                                    <div class="form-group col-12">
                                        ...
                                        <input type="text" class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}" id="name" {{ !isset($copy) && isset($product) ? 'disabled' : 'name=name' }} value="{{ old('name') ?? $product->name ?? '' }}" placeholder="輸入商品名稱">
                                        @if ($errors->has('name'))
                                        <span class="invalid-feedback" role="alert">
                                    ...
                                    <div class="form-group col-12">
                                        ...
                                        <input type="text" class="form-control {{ $errors->has('eng_name') ? ' is-invalid' : '' }}" id="eng_name" {{ !isset($copy) && isset($product) && in_array($product->status,[1,-3]) ? 'disabled' : 'name=eng_name' }} value="{{ old('eng_name') ?? $product->eng_name ?? '' }}" placeholder="輸入出口用英文名稱">
                                    ...
                                    <div class="form-group col-12">
                                        ...
                                        <input type="text" class="form-control {{ $errors->has('serving_size') ? ' is-invalid' : '' }}" id="serving_size" {{ !isset($copy) && isset($product) ? 'disabled' : 'name=serving_size' }} value="{{ old('serving_size') ?? $product->serving_size ?? '' }}" placeholder="輸入內容物或規格">
                                    ...
                                    <div class="form-group col-12">
                                        <label for="title">特色 (小標題) <span class="badge badge-info">此欄位將作為商品頁中字體放大的標題</span><small>(建議輸入13個中文字以內)</small></label>
                                        <input type="text" class="form-control {{ $errors->has('title') ? ' is-invalid' : '' }}" id="title" {{ !isset($copy) && isset($product) && in_array($product->status,[1,-3]) ? 'disabled' : 'name=title' }} value="{{ old('title') ?? $product->title ?? '' }}" placeholder="請用簡單文字(建議輸入13個中文字以內)說明商品特色">
                                        @if ($errors->has('title'))
                                        <span class="invalid-feedback" role="alert">
                                    ...
                                    <div class="form-group col-12">
                                        <label for="intro">簡介 (商品描述) <span class="badge badge-info">此欄位將作為商品頁中標題下方的簡單說明</span><small>(建議輸入70個中文字以內的簡單介紹)</small></label>
                                        <textarea rows="5" class="form-control {{ $errors->has('intro') ? ' is-invalid' : '' }}" id="intro" {{ !isset($copy) && isset($product) && in_array($product->status,[1,-3]) ? 'disabled' : 'name=intro' }} placeholder="輸入70個中文字以內的簡單介紹">{{ old('intro') ?? $product->intro ?? '' }}</textarea>
                                        @if ($errors->has('intro'))
                                        <span class="invalid-feedback" role="alert">
                            ...
                            <div class="col-6">
                                <div class="row">
                                    <div class="form-group col-3">
                                        ...
                                        <input type="number" class="text-danger text-bold form-control {{ $errors->has('price') ? ' is-invalid' : '' }}" id="price" {{ !isset($copy) && isset($product) ? 'disabled' : 'name=price' }} value="{{ old('price') ?? $product->price ?? '' }}" min="0" placeholder="輸入單價">
                                        @if ($errors->has('price'))
                                        <span class="invalid-feedback" role="alert">
                                    ...
                                    <div class="form-group col-3">
                                        ...
                                        <select class="form-control{{ $errors->has('unit_name_id') ? ' is-invalid' : '' }}" id="unit_name_id" {{ !isset($copy) && isset($product) && in_array($product->status,[1,-3]) ? 'disabled' : 'name=unit_name_id' }}>
                                    ...
                                    <div class="form-group col-3">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" class="form-control {{ $errors->has('gross_weight') ? ' is-invalid' : '' }}" id="gross_weight"  {{ !isset($copy) && isset($product) ? 'disabled' : 'name=gross_weight' }} value="{{ old('gross_weight') ?? (isset($copy) ? null : (isset($product) ? $product->gross_weight : '')) }}" min="0" placeholder="輸入毛重(g)">
                                    ...
                                    <div class="form-group col-3">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" class="form-control {{ $errors->has('net_weight') ? ' is-invalid' : '' }}" id="net_weight" {{ !isset($copy) && isset($product) ? 'disabled' : 'name=net_weight' }} value="{{ old('net_weight') ?? (isset($copy) ? null : (isset($product) ? $product->net_weight : '')) }}" min="0" placeholder="輸入產品淨重(g)">
                                            <div class="input-group-append">
                                                <div class="input-group-text"><i class="fab fa-goodreads-g"></i></div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="form-group col-4">
                                        ...
                                        <div class="input-group">
                                            <select class="form-control{{ $errors->has('is_tax_free') ? ' is-invalid' : '' }}" id="is_tax_free" {{ !isset($copy) && isset($product) && in_array($product->status,[1,-3]) ? 'disabled' : 'name=is_tax_free' }}>
                                    ...
                                    <div class="form-group col-6">
                                        ...
                                        <input type="number" class="form-control {{ $errors->has('storage_life') ? ' is-invalid' : '' }}" id="storage_life" value="{{ old('storage_life') ?? $product->storage_life ?? '' }}" min="0" placeholder="輸入保存天數" {{ !isset($copy) && isset($product) && in_array($product->status,[1,-3]) ? 'disabled' : 'name=storage_life' }}>
                                    ...
                                    <div class="form-group col-6">
                                        ...
                                        @endforeach
                                        @else
                                        <span class="text-danger text-bold">查無次分類資料。</span>
                                        @endif
                                        @else
                                    ...
                                    <div class="form-group col-3">
                                        ...
                                        <div class="input-group-text">
                                            {{ $vendor->product_sold_country }}
                                        </div>
                                        @foreach($countries as $country)
                                        @if($vendor->product_sold_country == $country->name)
                                        <input type="hidden" name="from_country_id" value="{{ $country->id }}">
                                        @endif
                                        @endforeach
                                    </div>
                                    <div class="form-group col-3">
                                            ...
                                            <div class="input-group">
                                                <select class="form-control{{ $errors->has('direct_shipment') ? ' is-invalid' : '' }}" id="direct_shipment" {{ !isset($copy) && isset($product) && in_array($product->status,[1,-3]) ? 'disabled' : 'name=direct_shipment' }}>
                                    ...
                                    <div class="form-group col-3">
                                        ...
                                        <input type="number" class="form-control {{ $errors->has('airplane_days') ? ' is-invalid' : '' }}" id="airplane_days" value="{{ old('airplane_days') ?? $product->airplane_days ?? '' }}" min="0" placeholder="輸入機場提貨指定天數" {{ !isset($copy) && isset($product) && in_array($product->status,[1,-3]) ? 'disabled' : 'name=airplane_days' }}>
                                    ...
                                    <div class="form-group col-3">
                                         ...
                                         <input type="number" class="form-control {{ $errors->has('hotel_days') ? ' is-invalid' : '' }}" id="hotel_days" value="{{ old('hotel_days') ?? $product->hotel_days ?? '' }}" min="0" placeholder="輸入旅店提貨指定天數" {{ !isset($copy) && isset($product) && in_array($product->status,[1,-3]) ? 'disabled' : 'name=hotel_days' }}>
                                    ...
                                    <div class="form-group col-12">
                                        ...
                                        <div>
                                            ...
                                            <div class="icheck-primary d-inline mr-3">
                                                <input type="checkbox"id="cachk{{ $shippingMethod->id }}" value="{{ $shippingMethod->id }}" {{ isset($product) ? in_array($shippingMethod->id,explode(',',$product->shipping_methods)) ? 'checked' : '' : '' }} {{ !isset($copy) && isset($product) && in_array($product->status,[1,-3]) ? 'disabled' : 'name=shipping_methods[]' }}>
                                    ...
                                    <div class="form-group col-12">
                                        ...
                                        <div class="select2-purple">
                                            <select class="select2" id="allow_country_ids" multiple="multiple" data-placeholder="選擇可寄送國家" data-dropdown-css-class="select2-purple" style="width: 100%;" {{ !isset($copy) && isset($product) && in_array($product->status,[1,-3]) ? 'disabled' : 'name=allow_country_ids[]' }}>
                                                @foreach($countries as $country)
                                    ...
                                    <div class="form-group col-6">
                                        ...
                                        <input type="text" class="datepicker form-control {{ $errors->has('vendor_earliest_delivery_date') ? ' is-invalid' : '' }}" id="vendor_earliest_delivery_date" value="{{ old('vendor_earliest_delivery_date') ?? $product->vendor_earliest_delivery_date ?? '' }}" min="0" placeholder="輸入廠商最快出貨日" {{ !isset($copy) && isset($product) && in_array($product->status,[1,-3]) ? 'disabled' : 'name=vendor_earliest_delivery_date' }}>
                                    ...
                                    <div class="form-group col-6">
                                        ...
                                        <input type="text" class="form-control datepicker {{ $errors->has('vendor_latest_delivery_date') ? ' is-invalid' : '' }}" id="vendor_latest_delivery_date" value="{{ $product->vendor_latest_delivery_date ?? '' }}" min="0" placeholder="輸入廠商最後出貨日" {{ !isset($copy) && isset($product) && in_array($product->status,[1,-3]) ? 'disabled' : 'name=vendor_latest_delivery_date' }}>
                            ...
                            @if(isset($product) && !isset($copy))
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" style="width: 5%" name="quantity" value="{{ $model->quantity ?? '' }}" min="0" placeholder="輸入庫存量" {{ in_array($product->status,[1,-3]) ? 'readonly' : '' }} required>
                                        <div class="input-group-prepend">
                                            <span class="input-group-text">安全庫存</span>
                                        </div>
                                        <input type="number" style="width: 5%" name="safe_quantity" value="{{ $model->safe_quantity ?? '' }}" min="1" placeholder="輸入安全庫存量" {{ in_array($product->status,[1,-3]) ? 'readonly' : '' }} required>
                                        <div class="input-group-prepend">
                                            <span class="input-group-text">廠商料號</span>
                                        </div>
                                        <input type="text" class="form-control" name="vendor_product_model_id" value="{{ $model->vendor_product_model_id ?? '' }}" min="1" placeholder="輸入廠商料號" {{ in_array($product->status,[1,-3]) ? 'readonly' : '' }}>
                                        <div class="input-group-append">
                                            <div class="input-group-text">
                                                國際條碼
                                            </div>
                                        </div>
                                        <input type="text" class="form-control" name="gtin13" value="{{ $model->gtin13 ?? '' }}" placeholder="輸入國際碼，共13碼數字" {{ in_array($product->status,[1,-3]) ? 'readonly' : '' }}>
                                        <div class="input-group-append">
                                            <div class="input-group-text">
                                            ...
                                            <tbody id="add_model">
                                                ...
                                                <tr class="add_model">
                                                    <td class="text-left align-middle">
                                                        <span class="input-group-text">{{ $product->model_name ?? '' }}</span>
                                                    </td>
                                                    <td class="text-left align-middle">
                                                        <input type="hidden" name="model_data[{{ $loop->iteration - 1 }}][name]" value="{{ $model->name ?? '' }}" readonly placeholder="範例：紅色" required>
                                                    ...
                                                    <td class="text-left align-middle">
                                                        <input type="number" class="form-control" name="model_data[{{ $loop->iteration - 1 }}][quantity]" value="{{ $model->quantity ?? '' }}" {{ in_array($product->status,[1,-3]) ? 'readonly' : '' }} min="0" placeholder="輸入庫存" required>
                                                    </td>
                                                    <td class="text-left align-middle">
                                                        <input type="number" class="form-control" name="model_data[{{ $loop->iteration - 1 }}][safe_quantity]"  value="{{ $model->safe_quantity ?? ''}}" {{ in_array($product->status,[1,-3]) ? 'readonly' : '' }} min="1" placeholder="輸入安全庫存" required>
                                                    </td>
                                                    <td class="text-left align-middle">
                                                    ...
                                                    <td class="text-left align-middle">
                                                        <input type="text" class="form-control" name="model_data[{{ $loop->iteration - 1 }}][gtin13]" value="{{ $model->gtin13 ?? ''}}" {{ in_array($product->status,[1,-3]) ? 'readonly' : '' }} placeholder="輸入國際碼，共13碼數字">
                                                    </td>
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            ...
                                                            <td class="align-middle">
                                                                <input type="hidden" class="form-control" name="packageData[{{ $loop->iteration -1}}][product_package_id]" value="{{ $package->id }}"  {{ in_array($product->status,[1,-3]) ? 'readonly' : '' }}>
                                                                <input type="text" class="form-control" name="packageData[{{ $loop->iteration -1 }}][name]" value="{{ $package->name }}" {{ in_array($product->status,[1,-3]) ? 'readonly' : '' }} placeholder="範例：紅色、尺寸大小" required>
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td class="align-middle">組合貨號</td>
                                                            <td class="align-middle">
                                                                <input type="hidden" name="packageData[{{ $loop->iteration -1 }}][sku]" value="{{ $package->sku }}" {{ in_array($product->status,[1,-3]) ? 'readonly' : '' }}>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="align-middle">
                                                                <input type="number" class="form-control" name="packageData[{{ $loop->iteration -1 }}][quantity]" value="{{ $package->quantity }}" {{ in_array($product->status,[1,-3]) ? 'readonly' : '' }} min="0" required>
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td class="align-middle">安全庫存</td>
                                                            <td class="align-middle">
                                                                <input type="number" class="form-control" name="packageData[{{ $loop->iteration -1 }}][safe_quantity]" value="{{ $package->safe_quantity }}" {{ in_array($product->status,[1,-3]) ? 'readonly' : '' }} min="1" required>
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                                ...
                                                <table class="table table-bordered table-sm mb-0">
                                                    <tbody id="add_product_{{ $loop->iteration -1 }}">
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td width="10%">
                                                                {{ $list->quantity }}
                                                                <input type="hidden" class="form-control form-control-sm" name="packageData[{{ $loop->parent->iteration -1 }}][list][{{ $loop->iteration -1 }}][quantity]" value="{{ $list->quantity }}" {{ in_array($product->status,[1,-3]) ? 'readonly' : '' }}>
                                                                <input type="hidden" name="packageData[{{ $loop->parent->iteration -1 }}][list][{{ $loop->iteration -1 }}][product_package_id]" value="{{ $package->id }}" {{ in_array($product->status,[1,-3]) ? 'readonly' : '' }}>
                                                                <input type="hidden" name="packageData[{{ $loop->parent->iteration -1 }}][list][{{ $loop->iteration -1 }}][product_model_id]" value="{{ $list->product_model_id }}" {{ in_array($product->status,[1,-3]) ? 'readonly' : '' }}>
                                                                <input type="hidden" name="packageData[{{ $loop->parent->iteration -1 }}][list][{{ $loop->iteration -1 }}][product_package_list_id]" value="{{ $list->id }}" {{ in_array($product->status,[1,-3]) ? 'readonly' : '' }}>
                                                            </td>
                                                        </tr>
                                                        @endforeach
                                                    </tbody>
                            ...
                            <div id="models" class="form-group col-md-12">
                                <div class="input-group">
                                    ...
                                    <input type="number" style="width: 10%" name="quantity" value="1" min="0" placeholder="輸入庫存量" required>
                                    <div class="input-group-prepend">
                                        <span class="input-group-text">安全庫存</span>
                                    </div>
                                    <input type="number" style="width: 10%" name="safe_quantity" value="1" min="1" placeholder="輸入安全庫存量" required>
                                    <div class="input-group-prepend">
                                        <span class="input-group-text">廠商料號</span>
                                    </div>
                            ...
                            <div class="form-group col-10">
                                <label for="description">詳細說明(商品規格)</span><span class="badge badge-secondary">此欄位將作為獨立說明頁面</span></label>
                                <textarea class="form-control {{ $errors->has('specification') ? ' is-invalid' : '' }} " rows="3" id="specification" placeholder="詳細說明(商品規格)..." required {{ !isset($copy) && isset($product) && in_array($product->status,[1,-3]) ? 'disabled' : 'name=specification' }}>{{ old('specification') ?? $product->specification ?? '' }}</textarea>
                            ...
                            <div class="form-group col-12">
                                <label for="photo">照片資料<span class="badge badge-danger">※ 檔案上傳後，若尺寸過大則自動等比縮小為 1440 x 760 (縮小後最大尺寸)。</span></label>
                                <div class="row">
                                    @for($i=0;$i<5;$i++)
                            ...
                            <div class="form-group col-6">
                                ...
                                @else
                                    <label for="verification_reason">狀態變更原因<span class="badge badge-secondary">例如審核退回之原因，或是下架時的備註</span></label>
                                    <h3 class="text-danger">{{ !empty($product->verification_reason) ? $product->verification_reason : '無' }}</h3>
                                @endif
                                @endif
                            </div>
                            @endif
                        ...
                        <div class="text-center bg-white">
                            ...
                            @else
                            <div><span class="text-danger">注意! 送審後將無法修改。</span></div>
                            @endif
                        </div>
                    </form>
                </div>
...
@else
{!! JsValidator::formRequest('App\Http\Requests\ProductsStoreRequest', '#myform'); !!}
@endif
                ...
                $.ajax({
                    ...
                    success: function(output) {
                        ...
                        if(output.length > 0){
                            ...
                            html += '<span class="text-danger text-bold">查無次分類資料。</span>';
                        }
                        $('#subcate').html(html);
                    }
                });
